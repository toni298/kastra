...
const load = async () => {
  ...
  try {
    ...
    if (current === requestId)
      error.value = requestError.response?.data?.message || 'Audit komisi gagal dimuat.'
  } finally {
...
const filteredItems = () =>
  items.value.filter(
    (item) => !search.value || item.invoice?.toLowerCase().includes(search.value.toLowerCase())
  )
</script>
    ...
    >
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-slate-500">{{ employee?.commission_type_label }}</p>
          </div>
        </div>
        <button
          type="button"
          class="rounded-lg p-2 text-slate-400 hover:bg-slate-100"
          aria-label="Tutup audit komisi"
          @click="emit('close')"
        >
          <X :size="20" />
        </button>
      ...
      <div class="flex-1 overflow-y-auto p-5">
        ...
        <div v-if="loading" class="flex min-h-56 items-center justify-center">
          <Spinner label="Memuat audit komisi..." />
        </div>
        ...
        >
          Tidak ada transaksi komisi.
        </div>
        <div
          v-else
          class="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-[#29476b] dark:border-[#29476b]"
        >
          <div v-for="item in filteredItems()" :key="item.id" class="space-y-2 p-4">
            <div class="flex items-center justify-between gap-3">
              <span class="font-mono text-sm font-semibold text-slate-900 dark:text-white">{{
                item.invoice
              }}</span
              ><span class="text-xs text-slate-500">{{ item.transaction_at || '-' }}</span>
            </div>
            <div class="grid grid-cols-3 gap-3 text-xs">
              <span class="text-slate-500"
                >Qty<strong class="mt-1 block text-slate-900 dark:text-white">{{
                  formatQty(item.qty)
                }}</strong></span
              ><span class="text-slate-500"
                >Omzet<strong class="mt-1 block text-slate-900 dark:text-white">{{
                  formatCurrency(item.total)
                }}</strong></span
              ><span class="text-slate-500"
                >Komisi<strong class="mt-1 block text-emerald-700">{{
