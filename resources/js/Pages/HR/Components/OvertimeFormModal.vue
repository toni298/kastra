...
const submit = async () => {
  ...
  try {
    await axios.post(route('hr.overtimes.store'), form.value)
    toast.success('Pengajuan lembur berhasil dicatat.')
    emit('saved')
    emit('close')
  } catch (error) {
    toast.error(error.response?.data?.message || 'Pengajuan lembur gagal disimpan.')
  } finally {
...
<template>
  <Modal
    :model-value="true"
    title="Catat Lembur Manual"
    description="Masukkan detail pengajuan lembur karyawan."
    size="lg"
  ...
  >
    ...
    <template #footer
      ...
      ><Button type="submit" form="overtime-form" :loading="processing"
        >Simpan Pengajuan</Button
      ></template
    >
  </Modal>
</template>
