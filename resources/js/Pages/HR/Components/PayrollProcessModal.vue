...
const submit = async () => { processing.value = true; try { await axios.post(route('hr.payrolls.generate'), form.value); toast.success('Draft payroll berhasil dibuat.'); emit('saved'); emit('close') } catch (error) { toast.error(error.response?.data?.message || 'Payroll gagal dibuat.') } finally { processing.value = false } }
</script>
<template>
