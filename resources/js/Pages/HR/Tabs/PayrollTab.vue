...
const fetchPayrolls = async () => {
  loading.value = true
  try {
    const response = await axios.get(route('hr.payrolls.index'), {
      params: {
...
const cards = computed(() => {
  ...
  const count = items.reduce(
    (sum, item) => sum + Number(item.employee_count || item.items_count || 0),
    0
  )
  return [
    ...
    { label: 'Jumlah Karyawan Terproses', value: count, icon: Check },
    {
      ...
      value: formatCurrency(count ? total / count : 0),
      icon: FileText,
    },
    {
      label: 'Status Posting Jurnal',
      value: items.some((item) => item.status === 'posted') ? 'Posted' : 'Draft',
      icon: Send,
    },
  ]
})
const postPayroll = async () => {
  ...
  try {
    ...
    toast.error(error.response?.data?.message || 'Payroll gagal diposting.')
  }
}
...
<template>
  <div class="space-y-5">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      <div>
        <h2 class="text-lg font-semibold text-slate-950 dark:text-white">Penggajian</h2>
        <p class="mt-1 text-sm text-slate-500">Kelola batch payroll dan slip gaji karyawan.</p>
      </div>
      <Button @click="processOpen = true"
        ><Plus :size="16" class="mr-2" />Proses Gaji Periode Baru</Button
      >
    </div>
    ...
    <DataPanel
      ><div class="relative">
        ...
        >
          <Spinner label="Memuat riwayat payroll..." />
        </div>
        <DataTable
          :items="rows.data || []"
          :pagination="rows"
          ...
          <tr v-for="row in rows.data" :key="row.id">
            <td class="px-5 py-4 font-semibold dark:text-white">
              {{ formatIndonesianDate(row.period) }}
            </td>
            <td class="px-5 py-4 dark:text-slate-200">
              {{ formatIndonesianDate(row.cutoff_date) }}
            </td>
            <td class="px-5 py-4 dark:text-slate-200">
              {{ row.employee_count || row.items_count || 0 }}
            </td>
            <td class="px-5 py-4 font-semibold dark:text-white">
            ...
            <td class="px-5 py-4">
              <Badge
                :variant="
                  row.status === 'posted' ? 'success' : row.status === 'draft' ? 'warning' : 'info'
                "
            ...
            <td class="px-5 py-4">
              <div class="flex justify-end gap-2">
                <button
                  type="button"
                  class="inline-flex items-center gap-1 text-sm font-semibold text-emerald-700"
                  @click="selected = row"
                >
                  <FileText :size="15" />Rincian</button
                ><button
                  v-if="row.status !== 'posted'"
                  type="button"
                  class="inline-flex items-center gap-1 text-sm font-semibold text-blue-700"
                  @click="posting = row"
                >
                  <Send :size="15" />Posting
                </button>
              </div>
            </td>
          </tr></DataTable
    ...
    <Modal
      v-if="posting"
      :model-value="true"
      title="Posting Jurnal Payroll"
      description="Batch payroll akan ditandai posted dan jurnal payroll akan dicatat."
      size="sm"
      @update:model-value="posting = null"
      ><p class="text-sm text-slate-600 dark:text-slate-300">
        Posting periode <strong>{{ formatIndonesianDate(posting.period) }}</strong> sebesar
        <strong>{{ formatCurrency(posting.total_amount) }}</strong
        >?
      </p>
      <template #footer
        ><Button variant="secondary" @click="posting = null">Batal</Button
        ><Button @click="postPayroll"
          ><Send :size="16" class="mr-2" />Posting Jurnal</Button
        ></template
      ></Modal
    >
  </div>
