...
<template>
  <Modal
    :model-value="true"
    title="Hapus Produk"
    description="Tindakan ini hanya memengaruhi data demo."
    size="md"
  ...
  >
    ...
    >
      <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-red-100 text-red-600"
        ><AlertTriangle :size="20"
      /></span>
      <div>
        <p class="font-medium text-red-900 dark:text-red-200">Hapus {{ item.product }}?</p>
        <p class="mt-1 text-sm leading-6 text-red-700 dark:text-red-300">
          Produk akan dihapus dari daftar stok sementara.
        </p>
      </div>
    </div>
    <template #footer
